...
function fillBracketSlot(slot, fighter) {
    const name = slot.querySelector(".bracket-node-name");
    const img = slot.querySelector(".bracket-node-photo");
    const label = fighter ? fighter.fighter_name : "—";
    ...
    if (fighter) {
        slot.classList.remove("bracket-node--vacant");
    } else {
        slot.classList.add("bracket-node--vacant");
    }
}
...
function renderTournament() {
    ...
    document.querySelectorAll(".bracket-match").forEach((match, index) => {
        ...
        const resultId = match.dataset.resultTarget;
        const metaId = match.dataset.metaTarget;
        const metaLabel = match.dataset.metaLabel;
        ...
        if (metaEl) {
            metaEl.textContent = node.winner ?
                `${metaLabel}: ${node.winner.fighter_name}` :
                "";
        }
    });
}
...
function playNextMatch(node) {
    ...
    // Head to the bottom of the tree first
    if (left && !left.winner) {
        const resolved = playNextMatch(left);
        if (resolved) {
            // Assign parent fighter as child winners
    ...
    // Assign parent fighter as child winners when available
    if (left && left.winner) node.setFighterA(left.winner);
    if (right && right.winner) node.setFighterB(right.winner);

    // Set winner if both fighters are ready
